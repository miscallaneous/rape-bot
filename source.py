...
cmd = 'mode 70,20'
os.system(cmd)
ctypes.windll.kernel32.SetConsoleTitleW("rape")
...
stream_url = "https://www.pornhub.com"
tts_language = "en"
...
def Init():
    ...
    try:
        ...
        os.system(f'title (misc selfbot) - Version {SELFBOT.__version__}')
...
colorama.init()
Nax = discord.Client()
Nax = commands.Bot(description='Nax Selfbot',
        ...
        if Nax.slotbot_sniper:
            if message.author.id == 346353957029019648:
                try:
                    ...
                    print(""
                          f"\n{Fore.RED}[{time} - rapebot Couldnt Grab]" + Fore.RESET)
                    SlotBotData()
                print(""
                      f"\n{Fore.RED}[{time} - rapebot Grabbed]" + Fore.RESET)
                SlotBotData()
        else:
            return
        ...
        try:
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
                if guild.id in Nax.whitelisted_users.keys() and i.user.id in Nax.whitelisted_users[
                    ...
                    await guild.ban(i.user, reason="misc Anti-Nuke")
        ...
        try:
            ...
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.bot_add):
                if member.guild.id in Nax.whitelisted_users.keys() and i.user.id in Nax.whitelisted_users[
                    ...
                    await guild.ban(member, reason="misc Anti-Nuke")
                    await guild.ban(i.user, reason="misc Anti-Nuke")
        except Exception as e:
            print(e)
...
async def supreme(ctx, *, args=None):
    ...
    if args is None:
        await ctx.send("missing parameters")
        return
    endpoint = "https://api.alexflipnote.dev/supreme?text=" + \
        args.replace(" ", "%20")
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
...
async def fax(ctx, *, args=None):
    ...
    if args is None:
        await ctx.send("missing parameters")
        return
    endpoint = "https://api.alexflipnote.dev/facts?text=" + \
        args.replace(" ", "%20")
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
